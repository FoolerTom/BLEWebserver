...
void loop() 
{
    ...
    if(tick100ms)
    {
        ...
        if((ms-startUpTime) < ((uint32_t)timerOn*1000)) //preset brightness for timerOn seconds
        {
            ...
            pwm_seq[0] = PWMperiod - (uint32_t)(PWMperiod * b_corr * (lightColor / 100.0f));
            pwm_seq[1] = PWMperiod - (uint32_t)(PWMperiod * b_corr * ((100 - lightColor) / 100.0f));
            NRF_PWM0->TASKS_SEQSTART[0] = 1;
        }
        else if((ms-startUpTime) < ((uint32_t)(timerOn+timerDim)*1000)) //dimming down for timerDim seconds
        {
            // Zeit seit Beginn des Dimmens
            uint32_t elapsed = ms - startUpTime - timerOn*1000;

            // 0..1 (1 = volle Helligkeit, 0 = aus)
            float fade = 1.0f - (float)elapsed / (float)(timerDim*1000);

            if (fade < 0.0f) fade = 0.0f;
            if (fade > 1.0f) fade = 1.0f;

            float gamma = 1.8f;
            float d_min = 25.0f / (float)PWMperiod;

            float t = (brightness - 1) / 99.0f;   // 0..1
            float t_gamma = powf(t, gamma);
            float b_corr = d_min + (1.0f - d_min) * t_gamma;

            // Jetzt herunterdimmen
            float b_dim = b_corr * fade;

            // PWM berechnen
            pwm_seq[0] = PWMperiod - (uint32_t)(PWMperiod * b_dim * (lightColor / 100.0f));
            pwm_seq[1] = PWMperiod - (uint32_t)(PWMperiod * b_dim * ((100 - lightColor) / 100.0f));
