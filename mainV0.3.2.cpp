...
void setupPWM()
{
    ...
    float gamma = 1.8f;
    float d_min = 25.0f / (float)PWMperiod;  // dein aktueller 1%-Wert

    float t = (brightness - 1) / 99.0f;   // 0..1
    float t_gamma = powf(t, gamma);
    float b_corr = d_min + (1.0f - d_min) * t_gamma;  // 1% -> d_min, 100% -> 1.0

    uint16_t targetWarmer = PWMperiod - (uint32_t)(PWMperiod * b_corr * (lightColor / 100.0f));
    uint16_t targetWarm = PWMperiod - (uint32_t)(PWMperiod * b_corr * ((100 - lightColor) / 100.0f));
...
void write32_callback(uint16_t conn_handle, BLECharacteristic* chr, uint8_t* data, uint16_t len)
{
    ...
    //WEBLOG("RX val=%lu id=%u \n", (unsigned long) val, (unsigned) id);
...
{
  static int32_t sollPWM0 = PWMperiod, sollPWM1 = PWMperiod;

  sollPWM0 = PWMperiod - 30 - (int64_t)PWMperiod * brightness * brightness / 10000 * lightColor * Dim / 10000;
  if(sollPWM0<0)
    sollPWM0 = 0;
  sollPWM1 = PWMperiod - 30 - (int64_t)PWMperiod * brightness * brightness / 10000 * (100 - lightColor) * Dim / 10000;
  if(sollPWM1<0)
    sollPWM1 = 0;

  if(sollPWM0>pwm_seq[0])
    pwm_seq[0]+=(sollPWM0-pwm_seq[0])/2;
  else if(sollPWM0<pwm_seq[0])
    pwm_seq[0]-=(pwm_seq[0]-sollPWM0)/2;
  if(sollPWM1>pwm_seq[1])
    pwm_seq[1]+=(sollPWM1-pwm_seq[1])/2;
  else if(sollPWM1<pwm_seq[1])
    pwm_seq[1]-=(pwm_seq[1]-sollPWM1)/2;
  NRF_PWM0->TASKS_SEQSTART[0] = 1;
}
