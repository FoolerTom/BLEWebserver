//Funktionstüchtig, noch nicht tiefer getestet
...
uint16_t pwm_seq[4] = {16000,16000,0,0};
...
void setupPWM()
{
    ...
    /*pwm_seq[0] = 16000 - (uint32_t)16 * brightness * lightColor / 10;
    pwm_seq[1] = 16000 - (uint32_t)16 * brightness * (100 - lightColor) / 10;*/
    startUpTime = millis();
...
void loop() 
{
    ...
    static uint32_t fallTime = 0, noShutDownTime = 0, batReadTicks = 0, loopTime = 0;
    ...
    if(tick100ms)
    {
        ...
        if(ms-startUpTime<2000) //soft start for 2 seconds
        {
            ...
            pwm_seq[0] = 16000 - (uint64_t)16 * brightness * lightColor * dimFactor / 10000;
            pwm_seq[1] = 16000 - (uint64_t)16 * brightness * (100 - lightColor) * dimFactor / 10000;
            NRF_PWM0->TASKS_SEQSTART[0] = 1;
        }
