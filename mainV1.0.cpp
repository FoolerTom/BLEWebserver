...
#define PWMperiodC      (PWMperiod-18)

#define WARM_PIN        12     //P1.12
#define WARMER_PIN      11     //P1.11
#define VBAT_PIN        31     // analogue battery sense P0.31
#define BUTTON_PIN      3      //P0.03

#define ENC_A_PIN       29     // P0.29
#define ENC_B_PIN       28     // P0.28
#define BAT_EN_PIN      14     // enable voltage divider for battery measurement P0.14
#define BAT_DISCON_PIN  13     // disconnect battery (pulls low to disconnect) P1.13
#define CHARGE_PIN      13     // control charging current P0.13
#define CHARGE_INDI_PIN 17     // P0.17
...
Adafruit_FlashTransport_QSPI QflashTransport;
...
int32_t brightness = 0, lightColor = 0, batteryLevel = 0, startUpBrightness = 0;
int32_t timerOn = 0, timerDim = 0;
uint32_t startUpTime = 0, ms = 0;
uint16_t pwm_seq[4] = {PWMperiod,PWMperiod,0,0};
int16_t saadc_buffer = 0;
volatile bool tick100ms = false, setStartUpBrightness = false;
...
void saveToFlash() 
{
  if(setStartUpBrightness)
    brightness = startUpBrightness | (1<<31);
...
void loadFromFlash() 
{
  ...
  if(timerOn+timerDim<60)
    timerOn = 60;
  setStartUpBrightness = (vars[2]>>31)&0x1;
  brightness = constrain(vars[2]&0x7FFFFFFF, 1, 100);
  if(setStartUpBrightness)
    startUpBrightness = brightness;
  lightColor = constrain(vars[3], 0, 100);

  QflashSleep();
}

void handleBrightness(uint32_t Dim=100)
{
  float t_gamma = powf(brightness/100.0f, 2.5f);

  pwm_seq[0] = PWMperiodC - (uint32_t)(PWMperiodC * t_gamma * (lightColor / 100.0f) * (Dim / 100.0f));
  pwm_seq[1] = PWMperiodC - (uint32_t)(PWMperiodC * t_gamma * ((100 - lightColor) / 100.0f) * (Dim / 100.0f));
...
{  
  ...
  for(uint8_t i=100; i>0; i--)  //dimming down for shutdown
  {
    handleBrightness(i);
    delay(15);
  }
...
void setupPWM()
{
  ...
  NRF_PWM0->PRESCALER   = PWM_PRESCALER_PRESCALER_DIV_2;          // 2 = 8MHz
  NRF_PWM0->COUNTERTOP  = PWMperiod;                              // 32000 = 250Hz
  ...
  NRF_PWM0->SEQ[0].CNT      = 4;       // Muss Mehrzahl von 4 sein.
  ...
  for(uint8_t i=1; i<=100; i++) //dimming up at start
  {
    handleBrightness(i);
    delay(15);
  }
...
void write32_callback(uint16_t conn_handle, BLECharacteristic* chr, uint8_t* data, uint16_t len)
{
  ...
  case 2:
    if(val&(1<<31))
    {
      brightness = startUpBrightness = val = constrain(val&0x7FFFFFFF, 1, 100);
      setStartUpBrightness = true;
      val |= (1<<31);
    }
    else
    {
      brightness = val = constrain(val, 1, 100); 
      setStartUpBrightness = false;
    }
    break;

  case 3: 
    lightColor = val = constrain(val, 0, 100); 
    break;
  ...
  case 99:
    ...
    delay(50);
    if(setStartUpBrightness)
      sendNotify(2, startUpBrightness | (1<<31));
    else
...
void loop() 
{
  ...
  if(tick100ms)
  {
    ...
    if((ms-startUpTime) < ((uint32_t)timerOn*1000)) //brightness according to setup for timerOn seconds
      handleBrightness();
    else if((ms-startUpTime) < ((uint32_t)(timerOn+timerDim)*1000)) //dimming down for timerDim seconds
      handleBrightness(100-((ms-startUpTime-timerOn*1000)/(timerDim*10)));
    else
      shutDown();
  }
